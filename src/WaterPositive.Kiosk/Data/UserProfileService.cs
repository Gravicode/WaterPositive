...
namespace WaterPositive.Kiosk.Data
{
    public class UserProfileService : ICrud<UserProfile>
    {
        ...
        public UserProfile GetDataById(object Id)
        {
            ...
            return default;
        } public UserProfile GetDataByUID(string UID)
        {
            ...
            try
            {
                ...
                foreach (var row in worksheet.Rows)
                {
                    if (counter > 0)
                    {
                        ...
                        newUserProfile.Username = string.IsNullOrEmpty(row.Cells[10].Value?.ToString().Trim()) ? MailHelper.GenerateEmailFromName(newUserProfile.FullName.Trim(), "WaterPositive.online") : row.Cells[10].Value?.ToString().Trim();
